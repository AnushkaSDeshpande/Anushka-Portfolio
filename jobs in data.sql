...
--  which 15 countries or locations have the most job opportunities in data according to the dataset. 
...
-- Salary statistics (average, minimum, and maximum salaries) based on size of company in the dataset.

select distinct(company_size) as Company_Size , round(avg(salary),0) as Average_salary , min(salary) as Minimum_salary, 
max(salary) as Maximum_salary
from jobs_data
group by 1;

--  Detailed breakdown of how many job entries exist for each job category within each distinct year 

select work_year,job_category, count(*) as Total_jobs
from jobs_data
group by 1,2
order by 1 asc;